...
impl greeter_server::Greeter for GreeterSvc {
    ...
    async fn say_hello_client_stream(&self, request: Request<tonic::Streaming<HelloRequest>>) -> Result<Response<HelloReply>, Status> {
        let mut s = request.into_inner();
        let mut names = vec![];
        while let Ok(Some(m)) = s.message().await { names.push(m.name); }
        Ok(Response::new(HelloReply { message: format!("Hello, {}", names.join(", ")) }))
    }

    type ChatStream = ReceiverStream<Result<HelloReply, Status>>;
    async fn chat(&self, request: Request<tonic::Streaming<HelloRequest>>) -> Result<Response<Self::ChatStream>, Status> {
        let mut s = request.into_inner();
        let (tx, rx) = mpsc::channel(8);
        tokio::spawn(async move {
            while let Ok(Some(m)) = s.message().await {
                let msg = HelloReply { message: format!("you said: {}", m.name) };

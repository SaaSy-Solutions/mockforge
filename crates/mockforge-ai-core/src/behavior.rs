...
use crate::context::StatefulAiContext;
use crate::llm_client::LlmClient;
use crate::rules::EvaluationContext;
use crate::types::{BehaviorRules, LlmGenerationRequest};
use crate::Result;
    ...
    async fn check_consistency_rules(
        ...
        for rule in &rules {
            if rule.matches(method, path) {
                // Apply rule action
                match &rule.action {
                    crate::rules::RuleAction::RequireAuth { message } => {
...
mod tests {
    use super::*;
    use crate::config::IntelligentBehaviorConfig;

...
pub struct BehaviorRules {
    ...
    #[serde(default)]
    pub consistency_rules: Vec<crate::rules::ConsistencyRule>,

    /// State machines for resource lifecycle management
    #[serde(default)]
    pub state_transitions: HashMap<String, crate::rules::StateMachine>,
...
/// Function type for providing the current time
///
/// This allows the caller to inject a custom time source (e.g., for time travel).
/// By default, `chrono::Utc::now()` is used.
pub type TimeFn = fn() -> DateTime<Utc>;

/// Default time function that returns the current UTC time
pub fn default_now() -> DateTime<Utc> {
    Utc::now()
}

impl SessionState {
    /// Create a new session state using the default time source
    pub fn new(session_id: impl Into<String>) -> Self {
        Self::new_with_time(session_id, default_now)
    }

    /// Create a new session state using a custom time source
    pub fn new_with_time(session_id: impl Into<String>, time_fn: TimeFn) -> Self {
        let now = time_fn();
        Self {
    ...
    /// Update last activity timestamp using the default time source
    pub fn touch(&mut self) {
        self.last_activity = Utc::now();
    }

    /// Update last activity timestamp using a custom time source
    pub fn touch_with_time(&mut self, time_fn: TimeFn) {
        self.last_activity = time_fn();
    }
    ...
    /// Check if session has been inactive for a duration using a custom time source
    pub fn is_inactive_with_time(&self, duration: chrono::Duration, time_fn: TimeFn) -> bool {
        time_fn().signed_duration_since(self.last_activity) > duration
    }
}

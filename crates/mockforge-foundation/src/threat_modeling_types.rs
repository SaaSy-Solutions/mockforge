...
//!
//! Pure data structures for API security threat assessments and remediation
//! suggestions. Extracted from
//! `mockforge-core::contract_drift::threat_modeling::types` so consumers
//! (http, ui, contracts) can reference threat-modeling data without depending
//! on deprecated core modules. The `ThreatAnalyzer` impl stays in core because
//! it depends on `OpenApiSpec` and LLM integration.

      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded shadow">
          ...
          <p className="text-sm text-gray-600">
            {payload.value} requests ({((payload.value / selectedMetric!.total_requests) * 100).toFixed(1)}%)
          </p>
        </div>
      );
    }
        ...
        {selectedMetric && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div className="space-y-1">
              <div className="text-2xl font-bold">{selectedMetric.total_requests.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Total Requests</div>
            </div>
            <div className="space-y-1">
              <div className="text-2xl font-bold text-green-600">{selectedMetric.success_count.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Successful</div>
            </div>
            <div className="space-y-1">
              <div className="text-2xl font-bold text-red-600">{selectedMetric.failure_count.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Failed</div>
            </div>
            <div className="space-y-1">
              <div className={`text-2xl font-bold ${selectedMetric.error_rate < 0.05 ? 'text-green-600' : selectedMetric.error_rate < 0.1 ? 'text-yellow-600' : 'text-red-600'}`}>
                {formatErrorRate(selectedMetric.error_rate)}
              </div>
          ...
          {selectedMetric && (
            <div className="flex justify-center space-x-6 mt-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm">Success ({((selectedMetric.success_count / selectedMetric.total_requests) * 100).toFixed(1)}%)</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 bg-red-500 rounded"></div>
                <span className="text-sm">Failure ({((selectedMetric.failure_count / selectedMetric.total_requests) * 100).toFixed(1)}%)</span>
              </div>
            </div>
          )}
        </div>

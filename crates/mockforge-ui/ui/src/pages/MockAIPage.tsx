...
export function MockAIPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        // In cloud mode, list explanations from the registry scoped to the
        // active workspace. Local mode hits the embedded admin server.
        let rulesCount = 0;
        if (isCloudMode()) {
          if (activeWorkspace?.id) {
            const rulesResponse = await aiStudioApi.listRuleExplanations(activeWorkspace.id);
        ...
        // Check if any OpenAPI specs have been generated by checking localStorage
        // Users can generate specs via the OpenAPI Generator page
      ...
      <Section title={t('page.mockai.quickActions')}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Button
            ...
            onClick={() => {
              toast.info(
                'Learn from Examples: Use POST /__mockforge/api/mockai/learn with example request/response pairs to train MockAI.',
                { duration: 5000 }
              );
            }}
            className="w-full"

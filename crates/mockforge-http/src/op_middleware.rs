...
use axum::{middleware::Next, response::Response, extract::State, Extension};
...
pub async fn fault_then_next<B>(State(shared): State<Shared>, Extension(op): Extension<OperationMeta>, req: Request<B>, next: Next) -> Response
where
    B: axum::body::HttpBody<Data = axum::body::Bytes> + Send + 'static,
    B::Error: Send + std::fmt::Display + std::error::Error + Sync,
{
    if let Some((code, msg)) = shared.profiles.maybe_fault(&op.id, &op.tags.iter().map(|s| s.to_string()).collect::<Vec<_>>()).await {
...
pub fn apply_overrides(shared: &Shared, op: &OperationMeta, body: &mut Value) {
    shared.overrides.apply(&op.id, &op.tags.iter().map(|s| s.to_string()).collect::<Vec<_>>(), body);
}

...
pub fn create_oauth2_client(config: &OAuth2Config) -> Result<oauth2::basic::BasicClient, Error> {
    ...
    let auth_url = oauth2::AuthUrl::new(config.auth_url.clone().unwrap_or_else(|| "https://example.com/auth".to_string()))
        .map_err(|e| Error::generic(format!("Invalid auth URL: {}", e)))?;

    let token_url = oauth2::TokenUrl::new(config.token_url.clone().unwrap_or_else(|| "https://example.com/token".to_string()))
        .map_err(|e| Error::generic(format!("Invalid token URL: {}", e)))?;

    Ok(oauth2::basic::BasicClient::new(client_id, Some(client_secret), auth_url, Some(token_url)))
}

...
impl DriftGitOpsHandler {
            ...
            if let Some(ref pr_config) = config.pr_config {
                if pr_config.enabled {
                    ...
                    let generator = match pr_config.provider {
                        crate::pr_generation::PRProvider::GitHub => {
                            PRGenerator::new_github(
                        ...
                        crate::pr_generation::PRProvider::GitLab => {
                            PRGenerator::new_gitlab(
                                pr_config.owner.clone(),
                                pr_config.repo.clone(),
                                token,
                                pr_config.base_branch.clone(),
    ...
    fn generate_pr_body(&self, incidents: &[DriftIncident]) -> String {
        ...
        body.push_str("This PR was automatically generated by MockForge to fix drift budget violations.\n\n");
        ...
        let breaking_count = incidents
            .iter()
            .filter(|i| matches!(i.incident_type, crate::incidents::types::IncidentType::BreakingChange))
            .count();

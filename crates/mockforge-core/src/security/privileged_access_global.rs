...
static GLOBAL_PRIVILEGED_ACCESS_MANAGER: Lazy<Arc<RwLock<Option<Arc<RwLock<PrivilegedAccessManager>>>>>> =
...
pub async fn init_global_privileged_access_manager(manager: Arc<RwLock<PrivilegedAccessManager>>) -> Result<(), crate::Error> {
    let mut global = GLOBAL_PRIVILEGED_ACCESS_MANAGER.write().await;
    if global.is_some() {
        return Err(crate::Error::Generic("Global privileged access manager already initialized".to_string()));
    }
    *global = Some(manager);

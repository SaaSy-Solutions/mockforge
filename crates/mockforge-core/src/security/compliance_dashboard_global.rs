...
static GLOBAL_COMPLIANCE_DASHBOARD_ENGINE: Lazy<Arc<RwLock<Option<Arc<RwLock<ComplianceDashboardEngine>>>>>> =
...
pub async fn init_global_compliance_dashboard_engine(engine: Arc<RwLock<ComplianceDashboardEngine>>) -> Result<(), crate::Error> {
    let mut global = GLOBAL_COMPLIANCE_DASHBOARD_ENGINE.write().await;
    if global.is_some() {
        return Err(crate::Error::Generic("Global compliance dashboard engine already initialized".to_string()));
    }
    *global = Some(engine);
...
pub async fn get_global_compliance_dashboard_engine() -> Option<Arc<RwLock<ComplianceDashboardEngine>>> {

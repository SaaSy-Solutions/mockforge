...
async fn test_record_handler_disabled() {
    ...
    handler.record_request(&fingerprint, 200, &response_headers, "{}", None)
        .await
        .unwrap();
...
async fn test_record_various_status_codes() {
    ...
    for status in status_codes {
        ...
        let recorded = handler.replay_handler()
...
async fn test_record_with_headers() {
    ...
    assert_eq!(recorded.response_headers.get("content-type"), Some(&"application/json".to_string()));
    assert_eq!(recorded.response_headers.get("x-custom-header"), Some(&"custom-value".to_string()));

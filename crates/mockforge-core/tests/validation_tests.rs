...
mod validation_tests {
    ...
    fn test_enhanced_schema_validation_integration() {
        use mockforge_core::schema_diff;
        ...
        let errors = schema_diff::validation_diff(&expected_schema, &valid_request);
        assert!(errors.is_empty(), "Valid request should not produce errors");
        ...
        let errors = schema_diff::validation_diff(&expected_schema, &invalid_request);
        ...
        let response_json = schema_diff::to_enhanced_422_json(errors);
        assert_eq!(response_json["error"], "Schema validation failed");
        assert!(response_json["validation_errors"].is_array());
        assert!(response_json["help"]["tips"].is_array());
    ...
    fn test_openapi31_content_encoding_validation() {
        ...
        // Valid base64
        let result = validator.validate_openapi_ext(&json!("SGVsbG8gV29ybGQ="), &schema);
        assert!(result.is_ok(), "Valid base64 should pass content encoding validation");

        // Invalid base64
        let result = validator.validate_openapi_ext(&json!("not-valid-base64!@#"), &schema);
        ...
        // Valid hex
        let result = validator.validate_openapi_ext(&json!("48656c6c6f"), &schema);
        assert!(result.is_ok(), "Valid hex should pass content encoding validation");

        // Invalid hex
        let result = validator.validate_openapi_ext(&json!("not-hex"), &schema);
    ...
    fn test_openapi31_composition_any_of() {
        ...
        let result = validator.validate_openapi_ext(&json!("hello"), &schema);
        ...
        let result = validator.validate_openapi_ext(&json!("hi"), &schema);
    ...
    fn test_openapi31_composition_one_of() {
        ...
        // Invalid: doesn't match any subschema
        let result = validator.validate_openapi_ext(&json!(42), &schema);
        assert!(result.is_err(), "Number should not match oneOf expecting strings");

        // Invalid: matches both subschemas (4 chars)
        let result = validator.validate_openapi_ext(&json!("test"), &schema);
    ...
    fn test_openapi31_composition_all_of() {
        ...
        let result = validator.validate_openapi_ext(&json!("this-is-a-very-long-string"), &schema);
    ...
    fn test_schema_diff_with_enhanced_error_reporting() {
        ...
        for error in &errors {
            if let Some(schema_info) = &error.schema_info {
                assert!(schema_info.data_type.len() > 0, "Error should include schema data type");
            }
        }
    }

...
impl SpecRegistry for KafkaSpecRegistry {
    ...
    fn operations(&self) -> Vec<SpecOperation> {
        ...
        self.fixtures.iter().map(|fixture| {
            SpecOperation {
    ...
    fn find_operation(&self, operation: &str, path: &str) -> Option<SpecOperation> {
        self.fixtures.iter().find(|fixture| {
            fixture.topic == path && operation == "PRODUCE"
        }).map(|fixture| {
            SpecOperation {
                name: fixture.identifier.clone(),
                path: fixture.topic.clone(),
    ...
    fn validate_request(&self, request: &ProtocolRequest) -> Result<ValidationResult> {
        ...
        Ok(ValidationResult {
            valid,
            errors: if valid { vec![] } else { vec![ValidationError {
        ...
        match operation.as_str() {
            "PRODUCE" => {
                let fixture = self.find_fixture_by_topic(topic)
                    .ok_or_else(|| mockforge_core::Error::generic(format!("No fixture found for topic {}", topic)))?;

                // Generate message using template
                let templating_context = mockforge_core::templating::TemplatingContext::with_env(request.metadata.clone());
                let value = self.template_engine.expand_tokens_with_context(&fixture.value_template, &templating_context);
                let _key = if let Some(key_pattern) = &fixture.key_pattern {
                    Some(self.template_engine.expand_str_with_context(key_pattern, &templating_context))
                } else {
                    None
                };
                ...
                Ok(ProtocolResponse {
                    ...
                    body: serde_json::to_string(&value).map_err(|e| mockforge_core::Error::Json(e))?.into_bytes(),
            ...
            "FETCH" => {
                ...
                let _offset = request.metadata.get("offset")
                ...
                Ok(ProtocolResponse {
                    ...
                    body: serde_json::to_vec(&messages).map_err(|e| mockforge_core::Error::Json(e))?,
                    content_type: "application/json".to_string(),
                })
            }

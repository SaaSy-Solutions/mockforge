...
impl ContractValidator {
    ...
    /// Whether optional fields are ignored
    pub fn is_ignore_optional_fields(&self) -> bool {
        self.ignore_optional_fields
    }

    /// Validate an OpenAPI spec against a live API endpoint
    ...
    pub fn compare_specs(
        &self,
        old_spec: &openapiv3::OpenAPI,
        new_spec: &openapiv3::OpenAPI,
    ) -> ValidationResult {
        let mut result = ValidationResult::new();

        // Check for removed endpoints
        for (path, _) in &old_spec.paths.paths {
